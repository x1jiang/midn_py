...
def infer_simice(models: list, df: pd.DataFrame, deterministic: bool = True) -> pd.DataFrame:
    """Impute missing values locally using returned SIMICE parameters (one per target)."""
    df = df.copy()
    for model in models:
