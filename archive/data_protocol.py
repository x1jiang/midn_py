"""
Unified wire schema — data structure definitions only.

REMARKS: Comments mark which algorithm(s) use each structure/message.

===== HIGHLIGHTED CHANGES =====
## CHANGE: All target indices are unified to 0-based (SIMICE originally used 1-based).
## CHANGE: Methods normalized to lowercase literals: "gaussian" | "logistic".
## CHANGE: A single 'iterate' message with a numeric 'mode' flag (1/2/0) replaces bespoke "Mode 1/2"/"Terminate".
## CHANGE: Matrices/arrays are float64 and serialized column-major (Fortran order) when flattened.
## CHANGE: Predictor dimension unified as 'p' across all protocols (some originals used 'q').
## CHANGE: HDMI concatenated parameters are split into named fields (beta_sel, beta_out, sigma, rho, vcov).
================================
"""
...
# ---- Common enums (used by all protocols) ----
Method = Literal["gaussian", "logistic"]        # ## CHANGE: normalized lowercase
Channel = Literal["control", "data"]
MessageType = Literal[
    "initialize", "finalize", "terminate",
    "request_info", "info",
    "iterate", "iter_response",
    "update_params", "ack",
    "impute",
    "get_final_data", "final_data"
]
