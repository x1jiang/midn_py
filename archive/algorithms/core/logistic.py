...
def Logit(X: np.ndarray, y: np.ndarray, offset: Optional[np.ndarray] = None, 
    ...
    if X.size == 0 or y.size == 0:
        print(f"⚠️ Logit: Empty input - X shape: {X.shape}, y shape: {y.shape}")
        p = X.shape[1] if X.ndim > 1 and X.shape[1] > 0 else 1
        return {
            'beta': np.zeros(p),
            'H': np.eye(p) * 1e-6,  # Small identity for numerical stability
            'converged': False,
            'iterations': 0
        }
    ...
    for iteration in range(maxiter):
        ...
        while True:
            ...
            # Check convergence - handle empty arrays
            if nbeta.size == 0 or beta.size == 0:
                print(f"⚠️ Logit: Empty beta arrays during iteration {iteration}")
                break
            elif np.max(np.abs(nbeta - beta)) < 1e-5:
                break
...
def SILogitNet(D: np.ndarray, idx: np.ndarray, yidx: int, remote_stats: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Federated logistic regression aggregation using Newton-Raphson.
    ...
    """
    print(f"🔬 SILogitNet: D shape: {D.shape}, idx length: {len(idx)}, yidx: {yidx}")
    print(f"🔬 SILogitNet: Remote stats count: {len(remote_stats)}")
    
    p = D.shape[1] - 1  # Assuming intercept column
    n = len(idx)
    
    if n == 0:
        print(f"⚠️ SILogitNet: No complete observations available")
        # When no local data, aggregate only remote statistics
        if not remote_stats:
            return {
                'beta': np.zeros(p),
                'H': np.eye(p) * 1e-6,
        ...
        # Aggregate only remote statistics
        ...
        lam = 1e-3
        H_reg = H_total + N * lam * np.eye(p) if N > 0 else np.eye(p) * 1e-6
    ...
    print(f"🔬 SILogitNet: After filtering - X shape: {X.shape}, y shape: {y.shape}")
    
    if X.size == 0 or y.size == 0:
        print(f"⚠️ SILogitNet: Empty arrays after filtering - using remote-only aggregation")
        # Same logic as above when no local data
        if not remote_stats:
            return {
                'beta': np.zeros(p),
                'H': np.eye(p) * 1e-6,
