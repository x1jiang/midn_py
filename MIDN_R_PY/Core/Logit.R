  ...
  while ( iter < maxiter )
  {
    ...
    if ( K > 0 )
      for ( k in 1:K )
      {
        ...
        print(paste0("SILogitNet Iteration ",iter,", communicating with site ",k, ", message Information"), flush=TRUE)
    ...
    # print(paste0("Iter=",iter,", Q=",round(Q,4),", step=",round(step,4),", max|dir|=",round(max(abs(dir)),6),", time=",round(Sys.time()-start_time,4),"s"), flush=TRUE)
    if ( max(abs(nbeta-beta)) < 1e-5 )
      break
    beta = nbeta
  }
