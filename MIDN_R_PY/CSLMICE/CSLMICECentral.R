...
CSLMICECentral = function(D,M,mvar,type,iter,iter0,hosts,ports,cent_ports)
{
  D = cbind(D,1)
  p = ncol(D)
  n = nrow(D)
    ...
    for ( it in 1:iter0 )
    {
      for ( i in 1:l )
      {
        ...
        if ( nmidx > 0 )
        {
          ...
          {
            fit.imp = CSLLSNet(D,cidx,j,rcons=rcons,wcons=wcons)
          ...
          {
            fit.imp = CSLLogitNet(D,cidx,j,rcons=rcons,wcons=wcons)
