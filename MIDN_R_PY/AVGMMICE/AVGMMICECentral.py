"""
Python implementation of AVGMMICECentral.R (Average Model MICE)

AVGMMICE combines chained equations with AVGMM aggregation across sites.
This mirrors SIMICE's Python structure but uses:
- Core.LS.AVGMLSNet for Gaussian targets
- Core.Logit.AVGMLogitNet for logistic targets
...
Entry point: avgmmice_central(D, config, site_ids, websockets)
...
from Core.LS import AVGMLSNet, ImputeLS
from Core.Logit import AVGMLogitNet, ImputeLogit
...
def _normalize_config(config: Optional[dict]) -> dict:
    ...
    if 'mvar' in raw:
        ...
        # Heuristic: if any index > 0 and caller says one_based or unspecified, treat as 0-based already
        # Prefer explicit one_based flag
        one_based = raw.get('one_based')
        if one_based is None:
            # If any index equals 0, assume 0-based; else assume already 0-based if small values present
            one_based = False
        if one_based:
...
async def avgmmice_central(D: np.ndarray,
    ...
    if not active_sites and remote_websockets:
        ...
        if debug:
            print(f"[AVGMMICE] No matching site_ids provided; using all connected: {active_sites}", flush=True)
    if debug:
        print(f"[AVGMMICE] Active sites: {active_sites}", flush=True)
    ...
    if debug:
        print(f"[AVGMMICE] Initializing remotes with targets (1-based) {[j+1 for j in mvar]}", flush=True)
    ...
    try:
        for m in range(1, M + 1):
            if debug:
                print(f"[AVGMMICE][imputation {m}/{M}] iterations={current_iters}", flush=True)
            for it in range(1, current_iters + 1):
                if debug:
                    print(f"[AVGMMICE] Iteration {it}/{current_iters}", flush=True)
                for i, j in enumerate(mvar):
                    if debug:
                        print(f"[AVGMMICE] Var j={j} type={type_list[i]}", flush=True)
                    ...
                    if type_list[i].lower() == "gaussian":
                        if debug:
                            print(f"[AVGMMICE] Requesting Gaussian info from remotes...", flush=True)
                        fit = await AVGMLSNet(D_aug, list(cidx), j,
                                              remote_websockets=remote_websockets,
                                              site_locks=site_locks,
                                              site_ids=active_sites)
                        # Sample sigma and coefficients
                        ...
                        # Cholesky may fail if not PD; add jitter fallback
                        try:
                            U = cholesky(vcov, lower=False)
                        except Exception:
                            eps = max(1e-10 * np.trace(vcov) / max(vcov.shape[0], 1), 1e-8)
                            U = cholesky(vcov + eps * np.eye(vcov.shape[0]), lower=False)
                        alpha = np.asarray(fit['beta']) + sig * (U.T @ np.random.normal(size=p_aug - 1))
                        ...
                        # Notify remotes
                        if debug:
                            print(f"[AVGMMICE] Sending Gaussian impute to remotes...", flush=True)
                        ...
                        if debug:
                            print(f"[AVGMMICE] Requesting Logistic info from remotes...", flush=True)
                        fit = await AVGMLogitNet(D_aug, list(cidx), j,
                                                 remote_websockets=remote_websockets,
                                                 site_locks=site_locks,
                                                 site_ids=active_sites)
                        vcov = np.asarray(fit['vcov'])
                        try:
                            ...
                            eps = max(1e-10 * np.trace(vcov) / max(vcov.shape[0], 1), 1e-8)
                            U = cholesky(vcov + eps * np.eye(vcov.shape[0]), lower=False)
                        alpha = np.asarray(fit['beta']) + (U.T @ np.random.normal(size=p_aug - 1))
                        Xmiss = np.delete(D_aug[midx, :], j, axis=1)
                        pr = 1.0 / (1.0 + np.exp(-(Xmiss @ alpha)))
                        D_aug[midx, j] = (np.random.random(size=midx.size) < pr).astype(float)

                        if debug:
                            print(f"[AVGMMICE] Sending Logistic impute to remotes...", flush=True)
        ...
        if debug:
            dur = time.time() - start
            print(f"[AVGMMICE] Completed {M} imputations in {dur:.3f}s", flush=True)
